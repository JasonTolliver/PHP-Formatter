...
function verifyText(want, got) {
    return '$this->verifyTextXXXXXOXOXOXOX("' + got.slice(got.indexOf('"') +1, got.lastIndexOf('"')) + '", ' + want + ');';
}
...
function verifyTextPresent(expression) {
    ...
    return  '$this->verifyTextPresentMOMMOMOMOMMYYYY("' + e.slice(e.indexOf('"') +1, e.lastIndexOf('"')) + '");';
}
